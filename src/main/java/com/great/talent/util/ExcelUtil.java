...
import com.great.talent.entity.Resume;
import com.great.talent.entity.Social;
...
public class ExcelUtil
{
	...
	public static void main(String[] args) {
		try {
			String fileName = "f:/jianli.xlsx";
			List<Object[]> list = importExcel(fileName);
			for (int i = 0; i < list.size(); i++) {
				Resume user = new Resume();
				Social social=new Social();
				Integer uid1=(Integer) list.get(i)[0];
				user.setUid(uid1);
				user.setResname((String) list.get(i)[1]);
				user.setSchoolname((String) list.get(i)[2]);
				List<Social> socials=new ArrayList<>();
				social.setSocialtime((String) list.get(i)[12]);
				social.setCompany((String) list.get(i)[13]);
				social.setContent((String) list.get(i)[14]);
				socials.add(social);
				user.setSocialList(socials);
